...
# مجموعة البوتات التي تم إيقافها يدوياً (حتى لا يعيد تشغيلها تلقائياً)
...
def stop_bot(bot_id: int) -> bool:
    """إيقاف بوت (يدوياً)
    يضيف البوت إلى قائمة الإيقاف اليدوي حتى لا يعيد تشغيله تلقائياً
    """
    # تسجيل أن هذا إيقاف يدوي
    manually_stopped.add(bot_id)
    ...
    db.update_bot(bot_id, status='stopped', pid=None)
    db.add_log(bot_id, "🛑 تم إيقاف البوت يدوياً", "warning")
    return True
...
def monitor_process(bot_id: int, process: subprocess.Popen):
    ...
    - إذا توقف البوت بسبب خطأ → يعيد تشغيله تلقائياً
    - إذا أوقفه المستخدم يدوياً → لا يعيد تشغيله
    """
    process.wait()
    ...
    # هل هذا توقف يدوي؟
    was_manual_stop = bot_id in manually_stopped
    if was_manual_stop:
        manually_stopped.discard(bot_id)
        db.update_bot(bot_id, status='stopped', pid=None)
        db.add_log(bot_id, "✅ البوت متوقف (إيقاف يدوي)", "info")
        return
...
def add_bot_from_token(name: str, token: str, description: str = "", user_id: Optional[int] = None) -> Optional[int]:
...
def add_bot_from_zip(name: str, token: str, zip_file_path: str, 
                     description: str = "", user_id: Optional[int] = None) -> Optional[int]:
...
def update_bot_from_zip(bot_id: int, zip_file_path: str) -> bool:
    """تحديث بوت موجود من ملف zip جديد
    - يحذف الكود القديم
    - يستخرج الكود الجديد
    - يحتفظ بقاعدة البيانات (data/)
    - يعيد الحقن (توكن + data_helper)
    - يعيد تشغيل البوت
    """
    bot_info = db.get_bot(bot_id)
    if not bot_info:
        print(f"❌ البوت {bot_id} غير موجود")
        return False
    ...
    if os.path.exists(data_dir):
        temp_data = os.path.join(bot_dir, "_data_backup")
    ...
    # 3. حذف الكود القديم
    if os.path.exists(code_dir):
        print(f"🗑️ حذف الكود القديم...")
        shutil.rmtree(code_dir)
    os.makedirs(code_dir, exist_ok=True)
    
    # 4. استخراج الكود الجديد
    print(f"📦 استخراج الكود الجديد...")
    try:
        with zipfile.ZipFile(zip_file_path, 'r') as zf:
            zf.extractall(code_dir)
    except zipfile.BadZipFile:
        print(f"❌ ملف ZIP تالف")
        # استعادة النسخة الاحتياطية
        if temp_data and os.path.exists(temp_data):
            if os.path.exists(data_dir):
                shutil.rmtree(data_dir)
            shutil.copytree(temp_data, data_dir)
            shutil.rmtree(temp_data)
        return False
    
    # 5. كشف المجلدات (نفس منطق extract_zip_bot)
    contents = [f for f in os.listdir(code_dir) 
                if not f.startswith('.') and not f.startswith('__')]
    if len(contents) == 1:
        single_item = os.path.join(code_dir, contents[0])
    ...
    if not found_entry:
        print(f"❌ ما لقينا أي ملف Python في الـ ZIP")
        if temp_data and os.path.exists(temp_data):
            if os.path.exists(data_dir):
                shutil.rmtree(data_dir)
            shutil.copytree(temp_data, data_dir)
    ...
    print(f"📄 نقطة الدخول: {found_entry}")
    ...
    if temp_data and os.path.exists(temp_data):
        ...
        shutil.copytree(temp_data, data_dir)
        shutil.rmtree(temp_data)
        print(f"💾 تم استعادة قاعدة البيانات")
    else:
        os.makedirs(data_dir, exist_ok=True)
    
    # 8. حقن data_helper + توكن
    _inject_credentials(code_dir, bot_info['token'], db.get_setting(bot_id, 'PREFIX', ''))
    _inject_data_helper(bot_dir)
    
    # نسخ data_helper داخل مجلد الكود
    code_helper = os.path.join(code_dir, "data_helper.py")
    if not os.path.exists(code_helper) and os.path.exists(DATA_HELPER_SOURCE):
        try:
            shutil.copy2(DATA_HELPER_SOURCE, code_helper)
    ...
    db.update_bot(bot_id, entry_point=found_entry)
    db.add_log(bot_id, f"🔄 تم تحديث البوت من ZIP (نقطة الدخول: {found_entry})", "success")
    ...
    # 11. إعادة تشغيل البوت
    print(f"🚀 إعادة تشغيل البوت...")
    start_bot(bot_id)
